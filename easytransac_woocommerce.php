...
function init_easytransac_gateway()
{
	class EasyTransacGateway extends WC_Payment_Gateway
	{
		function __construct()
		{
			...
			add_action('woocommerce_api_easytransac', array(
				$this,
		...
		function process_payment($order_id)
		{
			...
			foreach($order->get_items() as $item_id => $item_data) {
				...
				if ($product_type == 'subscription') $total_subscription+= $item_total;
			}
			...
			EasyTransacCoreLogger::getInstance()->setActive($this->get_option('debug_mode') == 'yes');
			EasyTransacCoreLogger::getInstance()->setFilePath(__DIR__ . '/logs/');
			EasyTransacCoreServices::getInstance()->provideAPIKey($api_key);
			if ($is_oneclick) {

				// SDK OneClick

				$transaction = (new EasyTransacEntitiesOneClickTransaction())->setAlias(strip_tags($_POST['oneclick_alias']))->setAmount(100 * $order->get_total())->setOrderId($order_id)->setClientId($this->getClientId());
				$dp = new EasyTransacRequestsOneClickPayment();
				try {
					$response = $dp->execute($transaction);
				}

				catch(Exception $exc) {
					EasyTransacCoreLogger::getInstance()->write('Payment Exception: ' . $exc->getMessage());
				}
				...
				if ($response->isSuccess()) {
					...
					if (in_array($doneTransaction->getStatus() , array(
						'captured',
						'pending'
					))) {
						...
						EasyTransacCoreLogger::getInstance()->write('Payment failed: ' . $response->getErrorCode() . ' - ' . $response->getErrorMessage());
				...
				$customer = (new EasyTransacEntitiesCustomer())->setEmail($address['email'])->setUid($order->get_user_id())->setFirstname($address['first_name'])->setLastname($address['last_name'])->setAddress($address['address_1'] . ' ' . $address['address_2'])->setZipCode($address['postcode'])->setCity($address['city'])->setBirthDate('')->setNationality('')->setCallingCode('')->setPhone($address['phone']);

				// If the order contain subscription product

				if (wcs_order_contains_subscription($order)) {
					$transaction = (new EasyTransacEntitiesPaymentPageTransaction())->setRebill('yes')->setDownPayment(100 * $order->get_total())->setAmount(100 * $total_subscription)->setCustomer($customer)->setOrderId($order_id)->setReturnUrl($return_url)->setCancelUrl($cancel_url)->setSecure($dsecure3)->setVersion($version_string)->setLanguage($language);
				}
				else {

					// If the order contain only "normal" products

					$transaction = (new EasyTransacEntitiesPaymentPageTransaction())->setAmount(100 * $order->get_total())->setCustomer($customer)->setOrderId($order_id)->setReturnUrl($return_url)->setCancelUrl($cancel_url)->setSecure($dsecure3)->setVersion($version_string)->setLanguage($language);
				}

				$request = new EasyTransacRequestsPaymentPage();
			...
			if (!$response->isSuccess()) {
				...
				EasyTransacCoreLogger::getInstance()->write('Payment error: ' . $response->getErrorCode() . ' - ' . $response->getErrorMessage());
		...
		function listcards()
		{
			...
			if (!$clientId) die(json_encode(array()));
			EasyTransacCoreServices::getInstance()->provideAPIKey($this->get_option('api_key'));
			$customer = (new EasyTransacEntitiesCustomer())->setClientId($clientId);
			$request = new EasyTransacRequestsCreditCardsList();
		...
		function check_callback_response()
		{
			...
			if ($is_https || (!$is_https && !empty($received_data))) {
				...
				try {
					$response = EasyTransacCorePaymentNotification::getContent($_POST, $api_key);
					...
					EasyTransacCoreLogger::getInstance()->write('Payment error: ' . $exc->getErrorCode() . ' (' . $exc->getMessage() . ') - ' . ($response instanceof EasyTransacResponsesStandardResponse ? $response->getErrorMessage() : ''));
			...
			EasyTransacCoreLogger::getInstance()->write('Received POST: ' . var_export($_POST, true));
		...
		function get_icon()
		{
			...
			if ($oneclick == 'yes') $icon_html.= '<script type="text/javascript" src="' . plugin_dir_url(__FILE__) . '/includes/oneclick.js"></script>';
